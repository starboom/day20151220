...
typedef struct Node
{
	int data;
	struct Node * next;
	struct Node * pre;//单项链表修改这里就可以变成双向链表
}Node;
/*
...
Node * CreatLb(int large)
{
	...
	head -> data = -100;
	head -> next = NULL;
	head -> pre  = NULL;//单项链表修改这里就可以变成双向链表
	Node * newnode;
	Node * curr;
	curr = head;
	...
	for(i = 0;i < large;i++)
	{
		...
		newnode -> data = i + 1;
		newnode -> next = NULL;
		newnode -> pre = NULL;//单项链表修改这里就可以变成双向链表


		curr -> next = newnode;
		newnode -> pre = curr;//单项链表修改这里就可以变成双向链表
...
void FreeLb(Node * head)
{
	...
	*/
	while(curr != NULL)
	{
		...
		if(curr != NULL)
		{
			curr -> pre = head;//单项链表修改这里就可以变成双向链表
		}
		free(tmp);
		tmp = NULL;
